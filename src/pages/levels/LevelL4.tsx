...
import { 
  CHAPTER8_PASSAGES, 
  getQuestionsByPassageId as getQuestionsByPassageIdC8
} from '../../data/questions/reading/chapter8';
import { 
  CHAPTER9_PASSAGES, 
  getQuestionsByPassageId as getQuestionsByPassageIdC9
} from '../../data/questions/reading/chapter9';

// Get L4 config
const LEVEL_CONFIG = getLevelConfig('L4') || LEVEL_CONFIGS[3];
...
const LevelL4 = () => {
  ...
  // 初始化题目 - Combine Chapter 8 and Chapter 9
  const initQuestions = useCallback(() => {
    // 获取 Chapter 8 的所有段落组
    const chapter8Groups = CHAPTER8_PASSAGES.map(passage => ({
      passage,
      questions: getQuestionsByPassageIdC8(passage.id)
    }));
    
    // 获取 Chapter 9 的所有段落组
    const chapter9Groups = CHAPTER9_PASSAGES.map(passage => ({
      passage,
      questions: getQuestionsByPassageIdC9(passage.id)
    }));
    
    // 合并所有段落，筛选难度 <= 3
    const allGroups = [...chapter8Groups, ...chapter9Groups]
      ...
      <div className="game-container">
        ...
        >
          ...
          <div className="card mt-8" style={{ maxWidth: '600px', margin: '32px auto', textAlign: 'left' }}>
            ...
            <ul style={{ paddingLeft: '20px', lineHeight: '2' }}>
              <li>✅ 阅读理解（Chapter 8-9，2章，4段，约15题）</li>
              <li>✅ 时态辨析题（5道语法选择题）</li>
              <li>⏱️ 答题时间：10分钟</li>
              <li>📊 及格分数：70分</li>
            </ul>
    ...
    <div className="game-container">
      ...
      >
        ...
        <p style={{ color: passed ? '#2a9d8f' : '#e76f51', marginTop: '8px', fontSize: '1.2rem' }}>
          {passed ? '你成功完成了 L4 测评！' : '继续加油，下次一定能通过！'}
        </p>
