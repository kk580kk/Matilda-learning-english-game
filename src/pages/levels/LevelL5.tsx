...
import { 
  CHAPTER10_PASSAGES, 
  getQuestionsByPassageId as getQuestionsByPassageIdC10
} from '../../data/questions/reading/chapter10';
...
const LevelL5 = () => {
  ...
  // 初始化题目 - Use Chapter 10 only
  const initQuestions = useCallback(() => {
    // 获取 Chapter 10 的所有段落组
    const allGroups = CHAPTER10_PASSAGES.map(passage => ({
      passage,
      questions: getQuestionsByPassageIdC10(passage.id)
    }));
    
    // 筛选难度 <= 3（目前所有段落难度3）
    const filteredGroups = allGroups.filter(group => group.passage.difficulty <= 3);
      ...
      <div className="game-container">
        ...
        >
          ...
          <div className="card mt-8" style={{ maxWidth: '600px', margin: '32px auto', textAlign: 'left' }}>
            ...
            <ul style={{ paddingLeft: '20px', lineHeight: '2' }}>
              <li>✅ 阅读理解（Chapter 10，1章，2段，7题）</li>
              <li>✅ 时态辨析题（5道语法选择题）</li>
              <li>⏱️ 答题时间：10分钟</li>
              <li>📊 及格分数：70分</li>
            </ul>
    ...
    <div className="game-container">
      ...
      >
        ...
        <p style={{ color: passed ? '#2a9d8f' : '#e76f51', marginTop: '8px', fontSize: '1.2rem' }}>
          {passed ? '你成功完成了 L5 测评！' : '继续加油，下次一定能通过！'}
        </p>
